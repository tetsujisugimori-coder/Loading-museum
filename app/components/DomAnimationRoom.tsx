...
function CreateRemove({ exhibit }: { exhibit: DomAnimationExhibit }) {
  ...
  const [operation, setOperation] = useState("待機中"); const removingRef = useRef(false); const reduced = useReducedMotion();
  const add = () => { const item = document.createElement("div"); item.className = "domCreatedItem"; item.textContent = ["✦", "●", "✧", "◆"][containerRef.current ? containerRef.current.children.length % 4 : 0]; containerRef.current?.append(item); setCount(containerRef.current?.children.length ?? 0); setOperation("追加"); };
  const remove = () => { const container = containerRef.current; const item = container?.lastElementChild as HTMLElement | null; if (!item || removingRef.current) return; removingRef.current = true; item.classList.add("domCreatedItemLeaving"); setOperation("退場中"); let finished = false; const finish = () => { if (finished) return; finished = true; item.remove(); removingRef.current = false; setCount(container?.children.length ?? 0); setOperation("DOMから削除"); }; if (reduced) { finish(); } else { item.addEventListener("transitionend", finish, { once: true }); window.setTimeout(finish, 360); } };
  const clear = () => { if (containerRef.current) containerRef.current.replaceChildren(); removingRef.current = false; setCount(0); setOperation("全削除"); };
  return <ExhibitCard exhibit={exhibit}><p className="domFinePrint">ボタンを押すたび、JavaScriptが新しいHTML要素を1個作り、展示ケースの中へ追加します。削除は退場classを付け、演出後にDOMからremoveします。</p><Stage><div className="domExternalLabel">補足: React管理外の専用コンテナ</div><div ref={containerRef} className="domExternalContainer" aria-label="DOM APIで追加される要素の領域" /></Stage><Controls><button type="button" onClick={add}>星を作って追加する</button><button type="button" onClick={remove} disabled={count === 0}>最後の星を取り除く</button><button type="button" onClick={clear} disabled={count === 0}>全削除</button><button type="button" onClick={clear}>リセット</button></Controls><Status>child count: {count} / 最後の操作: {operation} / API: class追加 → transitionend → remove</Status></ExhibitCard>;
}
...
function ScrollIntoViewDemo({ exhibit, reduced, destinationRef }: { exhibit: DomAnimationExhibit; reduced: boolean; destinationRef: RefObject<HTMLDivElement | null> }) {
  const [status, setStatus] = useState("移動先: #arrival-gate（未案内）");
  const guide = () => { const gate = destinationRef.current; gate?.scrollIntoView({ behavior: reduced ? "auto" : "smooth", block: "center" }); gate?.classList.add("domGateLit"); window.setTimeout(() => gate?.classList.remove("domGateLit"), reduced ? 0 : 1300); setStatus("移動先: #arrival-gate / 別の到着ゲートを指定しました"); };
  return <ExhibitCard exhibit={exhibit}><p className="domFinePrint">画面外にある要素を見える位置まで移動するDOM APIです。ボタン自身ではなく、見せたい別のHTML要素「到着ゲート」を指定するから意味があります。</p><Stage><button type="button" onClick={guide}>到着ゲートまで案内する</button><div className="domScrollSpacer">↓ 到着ゲートは展示グリッドの先です ↓</div></Stage><Status>{status}</Status></ExhibitCard>;
}
...
function LearningDemo({ exhibit }: { exhibit: DomAnimationExhibit }) {
  ...
  const action = (name: string) => {
    ...
    if (exhibit.id === "dom-text-content") { target.classList.remove("domTextChanged"); target.textContent = name; void target.offsetWidth; target.classList.add("domTextChanged"); window.setTimeout(() => target.classList.remove("domTextChanged"), 360); setState(`textContent: ${target.textContent}`); }
    ...
    if (exhibit.id === "dom-query-selector") { const found = target.querySelector(name) as HTMLElement | null; target.querySelectorAll(".domFound").forEach((node) => node.classList.remove("domFound")); found?.classList.add("domFound"); setState(`${name} → id=${found?.id || "なし"} / class=${found?.className || "なし"} / data-kind=${found?.getAttribute("data-kind") || "なし"}`); }
...
function Demo({ exhibit, active, reduced, destinationRef }: { exhibit: DomAnimationExhibit; active: boolean; reduced: boolean; destinationRef: RefObject<HTMLDivElement | null> }) {
  switch (exhibit.id) {
    ...
    case "dom-scroll-into-view": return <ScrollIntoViewDemo exhibit={exhibit} reduced={reduced} destinationRef={destinationRef} />;
...
export default function DomAnimationRoom() {
  ...
  const arrivalGateRef = useRef<HTMLDivElement>(null);
  ...
  return <section className="roomCard roomCardDom" aria-labelledby="dom-animation-room-title">
    ...
    <div id="dom-animation-room-panel" className="roomPanel" data-open={open} role="region" aria-labelledby="dom-animation-room-toggle" aria-hidden={!open} inert={!open}>
      <div className="roomPanelInner"><div className="domRoomBody">
        ...
        <div className="domExhibitGrid">{domAnimationExhibits.map((exhibit) => <Demo key={exhibit.id} exhibit={exhibit} active={open} reduced={reduced} destinationRef={arrivalGateRef} />)}</div>
        <div ref={arrivalGateRef} id="arrival-gate" className="domArrivalGate"><strong>ARRIVAL GATE</strong><span>scrollIntoView() が案内した、別のHTML要素です。</span></div>
