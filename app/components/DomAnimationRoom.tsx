...
function EventsDemo({ exhibit }: { exhibit: DomAnimationExhibit }) {
  ...
  return <ExhibitCard exhibit={exhibit}><p className="domFinePrint"><strong>やってみる:</strong> スタンプをクリック、光へカーソルを重ねる、Enter専用パネルをTabで選んでEnterを押します。タッチではスタンプを押して反応を確認できます。</p><Stage><button ref={clickRef} type="button" className="domEventStamp">✦<span>CLICK</span></button><div ref={hoverRef} className="domEventStamp" aria-label="ホバー用の光">●<span>HOVER</span></div><div ref={enterRef} tabIndex={0} role="button" className="domEventStamp" aria-label="Enterキー専用パネル">↵<span>ENTER</span></div></Stage><Status>何が変わった？ click: {counts.click} / pointerenter: {counts.hover} / keydown: {counts.enter} / 最後: {last}</Status><p className="domFinePrint">3つの学習用専用要素へaddEventListenerで登録し、アンマウント時にremoveEventListenerで解除します。Enter専用パネルは標準buttonのclickと重複しません。</p></ExhibitCard>;
}
