...
export const domAnimationExhibits: readonly DomAnimationExhibit[] = [
  ...
  { id: "dom-attributes", title: "Attributes — 操作パネル", description: "属性はHTML要素が持つ追加情報・設定値です。disabledとaria-expandedを変え、操作可否と開閉状態を対応させます。", apiNames: ["setAttribute", "removeAttribute"], code: 'button.setAttribute("disabled", "");\nbutton.removeAttribute("disabled");', category: "style" },
  { id: "dom-events", title: "addEventListener — 反応を待つ", description: "ユーザー操作などの出来事を待ち、起きたら処理を実行する入口です。クリック、ホバー、Enterで反応します。", apiNames: ["addEventListener"], code: 'target.addEventListener("click", react);', category: "style" },
  ...
  { id: "dom-scroll-into-view", title: "scrollIntoView() — 新しい展示へ案内", description: "明確なボタン操作の結果として、指定した要素が見える位置までスクロールします。", apiNames: ["element.scrollIntoView"], code: 'newExhibit.scrollIntoView({ behavior: "smooth" });', category: "measurement" },
