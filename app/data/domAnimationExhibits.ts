...
export const domAnimationExhibits: readonly DomAnimationExhibit[] = [
  ...
  { id: "dom-create-remove", title: "Create and Remove Element", description: "React管理外と明示した専用コンテナだけで、要素の追加・退場・削除を試します。", apiNames: ["document.createElement", "append", "classList.add", "remove"], code: 'item.classList.add("domCreatedItemLeaving");\nitem.addEventListener("transitionend", () => item.remove(), { once: true });', category: "structure" },
  ...
  { id: "dom-query-selector", title: "querySelector — 展示物を探す", description: "ID・class・属性のCSSセレクタで、DOM内から最初に一致する展示物を探し、スポットライトを当てます。", apiNames: ["querySelector"], code: 'const found = room.querySelector(selector);\n// #featured-exhibit / .museum-artifact / [data-kind="signal"]', category: "measurement" },
  ...
  { id: "dom-scroll-into-view", title: "scrollIntoView() — 到着ゲートへ案内", description: "長いページで画面外にある目的地を見つけに行くため、ボタン自身ではない別のHTML要素を見える位置までスクロールします。", apiNames: ["element.scrollIntoView"], code: 'destinationRef.current?.scrollIntoView({\n  behavior: reduced ? "auto" : "smooth",\n  block: "center",\n});', category: "measurement" },
