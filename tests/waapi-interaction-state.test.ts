...
describe("WAAPI interaction state machines", () => {
  it("parry/dodge timing has early, success, and late boundaries", () => {
    expect(judgeTiming(419)).toBe("EARLY");
    expect(judgeTiming(420)).toBe("PERFECT");
    expect(judgeTiming(680)).toBe("PERFECT");
    expect(judgeTiming(681)).toBe("LATE");
  });

  it("AI card and comparison phases terminate", () => {
    expect(nextAiState("Idle")).toBe("Starting");
    expect(nextAiState("Reasoning / Tool")).toBe("Writing");
    expect(nextAiState("Verifying")).toBe("Complete");
    expect(nextAiState("Complete")).toBe("Complete");
    let phase: Parameters<typeof nextComparisonPhase>[0] = "Idle";
    for (let index = 1; index < aiPhases.length; index += 1) phase = nextComparisonPhase(phase);
    expect(phase).toBe("Complete");
    expect(nextComparisonPhase("Error")).toBe("Idle");
  });
});
