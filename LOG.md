...
- 実装量に対して展示テーマ、史実と創作の境界、観察点、操作結果の意味が伝わりにくかったApple創成期展示室を、Apple IからApple II、Disk IIへ進む技術史として再設計した。
- 説明だけに存在したLOAD失敗とエラー選択を実装し、表示と操作の不一致を解消した。

### 入口題名と導入文

- 曖昧で史実上も不正確だった「完成品になる前のコンピュータと、家庭へ開かれた『動き』」を削除した。
- 正式展示室名は「Apple創成期展示室」のまま、主題を「Apple IからApple IIへ」、副題を「文字・カセット・カラー・ディスクが変えたコンピュータとの対話」、分類アイブロウを`ROOM / 1976–1979`へ整理した。
